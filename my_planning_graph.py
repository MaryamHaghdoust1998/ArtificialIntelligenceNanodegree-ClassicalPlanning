...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        t_v = 0
        for a_action in self.children[actionA]:
            for b_action in self.children[actionB]:
                if a_action == ~b_action or b_action == ~a_action:
                    t_v += 1
        if t_v > 0:
            return True
        else:
    ...
    def _interference(self, actionA, actionB):
        ...
        val = 0
        vall = 0
        for m in self.children[actionA]:
            for n in self.parents[actionB]:
                if m == ~n:
                    val += 1
                else:
                    val += 0
        for g in self.children[actionB]:
            for h in self.parents[actionA]:
                if g == ~h:
                    vall += 1
                else:
                    vall += 0
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        # TODO: implement this function
        v_t = 0
        for pa in self.parents[actionA]:
            for pb in self.parents[actionB]:
                if self.parent_layer.is_mutex(pa, pb) and self.parent_layer.is_mutex(pb, pa):
                    v_t += 1
        if v_t > 0:
            return True
        else:
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        while not self._is_leveled:
            goals.difference_update(self.literal_layers[-1])
            if goal_len != len(goals):
                level_sum += level_count * (goal_len - len(goals))
                goal_len = len(goals)
    ...
    def h_setlevel(self):
        ...
        goals = self.goal.copy()
        while not self._is_leveled:
            if goals.issubset(self.literal_layers[-1]) and not \
                    any([self.literal_layers[-1].is_mutex(goal_a, goal_b) for goal_a in goals
                         for goal_b in goals if goal_a != goal_b]):
                break
            else:
                self._extend()
                level_counter += 1
